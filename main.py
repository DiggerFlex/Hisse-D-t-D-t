...
@app.route('/')
def home():
    return "⚡ NASDAQ REALTIME TERMINAL ACTIVE ⚡"
            ...
            for update in res["result"]:
                ...
                if "message" in update and "text" in update["message"]:
                    ...
                    if text == "/stop":
                        ...
                        process_symbol("CDTG", force_send=True)
                        ...
                        durum_msg = (
                            ...
                            f"📊 *Günlük Sinyal:* `{len(gunluk_sinyaller)} Adet`\n"
                            f"⚡ *Gecikme:* `{latency:.0f} ms`\n"
                            f"🚀 *Veri Kaynağı:* `Polygon Real-Time API`"
                        )
                        send_telegram_msg(durum_msg)
...
# ==========================================
# 5. POLYGON REAL-TIME ANLIK TARAMA
# ==========================================
def process_symbol(symbol, force_send=False):
    try:
        url = f"https://api.polygon.io/v2/aggs/ticker/{symbol}/range/1/minute/{datetime.date.today()}/{datetime.date.today()}?adjusted=true&sort=desc&limit=5&apiKey={POLYGON_API_KEY}"
        res = requests.get(url, timeout=5).json()

        if "results" not in res or len(res["results"]) < 2:
            return
        ...
        high_price = last_bar["h"]
        open_price = last_bar["o"]
        ...
        if not force_send:
            ...
            is_strong_bar = (last_price - open_price) / open_price >= 0.01

            if not (price_change_pct >= 1.2 and is_strong_bar):
                return
...
def gun_sonu_raporu_gonder():
    ...
    for symbol, data in gunluk_sinyaller.items():
        try:
            url = f"https://api.polygon.io/v2/aggs/ticker/{symbol}/range/1/day/{datetime.date.today()}/{datetime.date.today()}?apiKey={POLYGON_API_KEY}"
            res = requests.get(url, timeout=5).json()
            
            entry = data['entry']
            zirve = res["results"][0]["h"] if "results" in res and res["results"] else entry
...
def canli_kesintisiz_tarama():
    ...
    now_ts = time.time()
    # 900 saniye = 15 Dakikada bir durum kontrol mesajı
